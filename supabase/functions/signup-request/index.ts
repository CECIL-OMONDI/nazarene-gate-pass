...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!/^[+0-9 \-]{7,20}$/.test(phone)) return j({ error: "Valid phone number is required" }, 400);
    ...
    // collision: phone already used by an active account or pending request
    ...
    const { error } = await admin.from("signup_requests").insert({
      full_name, phone, email, requested_role: role, password, status: "pending",
    });
