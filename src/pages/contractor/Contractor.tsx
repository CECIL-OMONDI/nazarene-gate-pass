...
export function ContractorHome() {
  ...
  return (
    <AppShell title="My Sites">
      {sites.length === 0 && <div className="text-muted-foreground">You have no sites assigned yet.</div>}
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sites.map(s => (
          <Link key={s.id} to={`/contractor/site/${s.id}`}>
...
export function ContractorSiteDetail() {
  ...
  return (
    <AppShell title={site?.name ?? "Site"}>
      <Link to="/contractor" className="text-sm text-primary hover:underline">← All sites</Link>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
        ...
        <Card><CardHeader><CardTitle>Workers</CardTitle></CardHeader><CardContent className="space-y-2">
          <div><Label>Skilled</Label><Input type="number" min="0" value={workers.skilled_count} onChange={e=>setWorkers({...workers, skilled_count: Number(e.target.value)})} /></div>
          <div><Label>Unskilled</Label><Input type="number" min="0" value={workers.unskilled_count} onChange={e=>setWorkers({...workers, unskilled_count: Number(e.target.value)})} /></div>
          <Button onClick={saveWorkers} size="sm" className="w-full">Save</Button>
        </CardContent></Card>
        <ToolsCard siteId={id!} tools={tools} reload={load} />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-4">
        <PlaceOrderCard siteId={id!} materials={materials} reload={load} />
        <Card><CardHeader><CardTitle>Open Orders</CardTitle></CardHeader><CardContent className="space-y-3">
...
function ToolsCard({ siteId, tools, reload }: { siteId: string; tools: any[]; reload: () => void }) {
    ...
    <Card><CardHeader><CardTitle>Tools</CardTitle></CardHeader><CardContent className="space-y-2">
      ...
      <ul className="text-sm divide-y">
        {tools.map(t => <li key={t.id} className="flex items-center justify-between py-1.5">
          <span>{t.name}</span><span className="flex items-center gap-2"><span className="font-mono">{t.quantity}</span><Button variant="ghost" size="icon" className="h-6 w-6" onClick={()=>del(t.id)}><Trash2 className="h-3 w-3"/></Button></span>
        </li>)}
      </ul>
    </CardContent></Card>
  );
